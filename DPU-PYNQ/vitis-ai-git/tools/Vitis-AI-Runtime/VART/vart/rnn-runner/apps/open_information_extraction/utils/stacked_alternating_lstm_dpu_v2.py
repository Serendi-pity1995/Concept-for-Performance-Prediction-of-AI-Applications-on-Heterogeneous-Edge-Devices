...
class StackedAlternatingLstm(torch.nn.Module):
    ...
    def __init__(
        ...
        assert(device_name in ("U50LV", "U25")
               ), "TARGET_DEVICE should be U50LV/U25"
        num_sequences = -1
        device_core_id = 0
        self.thread_lstm = []
        self.batch = []
        ...
        for i in range(core):
            ...
            self.thread_lstm.append(vart.Runner.create_runner(
                xgraph.get_root_subgraph(), "run"))
    ...
    def run(self, input_list, frame_size, output_list, frame_num, i):
        ...
        self.thread_lstm[i].run(input_list.flatten(),
                                frame_size, output, frame_num)
    ...
    def forward(
        ...
        layer_output = np.empty(
            shape=(flow_batch, frame_num, 300), dtype=np.int16)
        sw_input = torch.floor(sequence_tensor.data *
                               self.input_scale).cpu().short().numpy()
        dpu_in = [np.zeros((b, frame_num, 224), dtype=np.int16)
                  for b in self.batch]
        dpu_out = [np.zeros((b, frame_num, 320), dtype=np.int16)
                   for b in self.batch]
#        print(hw_input.shape)
        count = 0
        # hw_input : (full_batch, #frame, 224)
        # layer_output : (full_batch, #frame, 300)
        #
        while count < flow_batch:
            if (count + self.total_batch) <= flow_batch:
                for i, b in enumerate(self.batch):
                    dpu_in[i][:, :, :200] = sw_input[count:count+b]
                    jid = self.thread_lstm[i].execute_async(
                        [dpu_in[i].reshape(b, frame_num, 224)],
                        [dpu_out[i].reshape(b, frame_num, 320)],
                        True)
                    self.thread_lstm[i].wait(jid)
                    layer_output[count:count+b] = dpu_out[i][:, :, :300]
                    count = count + b
            elif flow_batch - count > self.batch[0]:
                b = self.batch[0]
                dpu_in[0][:, :, :200] = sw_input[count:count+b]
                jid = self.thread_lstm[0].execute_async(
                    [dpu_in[0][:b].reshape(b, frame_num, 224)],
                    [dpu_out[0].reshape(b, frame_num, 320)],
                    True)
                self.thread_lstm[0].wait(jid)
                layer_output[count:count+b] = dpu_out[0][:, :, :300]

                rest_batch = flow_batch-count-b
                dpu_in[1][:rest_batch, :, :200] = sw_input[count+b:]
                jid = self.thread_lstm[1].execute_async(
                    [dpu_in[1][:rest_batch].reshape(
                        rest_batch, frame_num, 224)],
                    [dpu_out[1][:rest_batch].reshape(
                        rest_batch, frame_num, 320)],
                    True)
                self.thread_lstm[1].wait(jid)
                layer_output[count+b:] = dpu_out[1][:rest_batch, :, :300]
                count = flow_batch
            else:
                # pdb.set_trace()
                rest_batch = flow_batch - count
                b = rest_batch
                dpu_in[0][:rest_batch, :, :200] = sw_input[count:]
                jid = self.thread_lstm[0].execute_async(
                    [dpu_in[0][:b].reshape(b, frame_num, 224)],
                    [dpu_out[0][:b].reshape(b, frame_num, 320)],
                    True)
                self.thread_lstm[0].wait(jid)
                layer_output[count:] = dpu_out[0][:rest_batch, :, :300]
...
#            layer_output.append(output.reshape(frame_num, 300))
        torch_output = torch.from_numpy(
            np.array(layer_output, dtype=np.float32)/self.output_scale)
