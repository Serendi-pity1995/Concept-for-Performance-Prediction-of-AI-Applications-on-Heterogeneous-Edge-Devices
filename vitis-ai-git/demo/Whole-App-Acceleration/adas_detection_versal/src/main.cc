...
int runTotal;
PPHandle* handle;
void setInputImageForYOLO(float *data, const Mat& frame) {
...
void readFrame(vector<string> images, vart::Runner* runner){
  ...
  while(true){ 
    if(idxInputImage< images.size()) 
    {
      ...
      std::cout<<"Image read "<<baseImagePath + images[idxInputImage]<<std::endl;
      mtxQueueInput.lock();
      queueInput.push(make_pair(idxInputImage++, img));
	  
	  /* feed input frame into DPU Task with mean value */
      setInputImageForYOLO(data, img);
...
void runYOLO(vart::Runner* runner) {
  ...
  while (true) {
    ...
    /* feed input frame into DPU Task with mean value */
    //setInputImageForYOLO(data, pairIndexImage.second);
...
int main(const int argc, const char** argv) {
  if (argc != 3) {
    cout << "Usage of ADAS detection: ./adas path(for xmodel file) 1(usePPFlag)" << endl;
    return -1;
  }
  ...
  std::cout<<"Num of images = "<<images.size()<<std::endl;
  usePPFlag = stoi(argv[2])==1;

  /* Load xclbin for hardware pre-processor */
  if (usePPFlag){
    std::cout<<"Running with HW Accel pre-process"<<std::endl;
    pp_kernel_init(handle,"/media/sd-mmcblk0p1/dpu.xclbin", "pp_pipeline_accel",0);
  }
