		...
		int org_wt)
{
	...
	//# Get pp output physical address
	uint64_t pp_output_dev_addr = pphandle->pp_output_dev_addr;
    ...
    int dx = (org_wt - out_width_resize)/2;
    int dy = (org_ht - out_height_resize)/2;
	...
	try {
		...
		ecmd2->data[XFPPPre_CONTROL_ADDR_img_out/4] = pp_output_dev_addr & 0xFFFFFFFF;
		ecmd2->data[XFPPPre_CONTROL_ADDR_img_out/4 + 1] = (pp_output_dev_addr >> 32) & 0xFFFFFFFF;
		...
		ecmd2->data[XFPPPre_CONTROL_ADDR_img_out/4] = pp_output_dev_addr;	
